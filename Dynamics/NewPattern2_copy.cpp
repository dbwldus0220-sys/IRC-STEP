...
MatrixXd Trajectory::RF_xsimulation_straightwalk(){
	...
	for (int i = 0; i < sim_n; i++)
	{
		...
		else if (i < 1.1 * dwalktime_n) Footpos[i] = step;	// 1보 (보행 시작: 오른발만 앞으로)
		...
		}	// 2보 및 3보 (중간 보행: 추월하며 전진)
		
		else if (i < (Rfootflag + 1.1) * dwalktime_n) Footpos[i] = (2 * Rfootflag + 1) * step;
		else Footpos[i] = (2 * step_n - 3) * step;	// 마지막 보 (보행 종료: 양발 모으기)

		if ((i + 1) % walktime_n == 0)	// 한 걸음 주기(walktime_n)가 끝날 때마다
		{
			if (Rfootflag < step_n - 2) Rfootflag = Rfootflag + 1;	// 플래그를 1씩 증가
		}
	};
	return Footpos;
};
...
MatrixXd Trajectory::YComSimulation_Sidewalk(double a, double b, double c, double d, double e, double f)
{
	...
	for (int i = 0; i < sim_n + 19; i++)
	{
		...
		YCom.col(i + 1) = A * YCom.col(i) + B * u[i];	// 1. 상태 업데이트 방정식 (State Transition Equation)

		zmp[i] = C * YCom.col(i);	// 2. 출력 방정식 (Output Equation)
